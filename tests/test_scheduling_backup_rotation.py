...
def test_backup_used_when_primary_unavailable(db_session, models):
    ...
    # Use a future date to avoid the scheduling buffer
    future_date = (datetime.now() + timedelta(days=5)).replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    # Primary has time off on the event date
